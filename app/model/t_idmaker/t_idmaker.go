...
// 获取单个数据
func GetOne(key string) (rspInfo *Entity) {
	r, err := Model.FindOne(g.Map{"key": key})
	if err != nil {
...
// 更新id
func UpdateId(key string, value int) error {
	return ModifyBase(g.Map{"value": value},
		&Entity{
			Key: key,
		})
}
...
func ModifyBase(updateInfo interface{}, info *Entity) error {
	r, err := Model.Data(updateInfo).Where(g.Map{"key": info.Key}).Update()
	if err != nil {
