...
// 获取单个数据(where phone=？)
func GetOneByPhone(key string) (rspInfo *Entity) {
	r, err := Model.FindOne(g.Map{"phone": key})
	if err != nil {
...
// 获取单个数据(where aid=？)
func GetOneByAid(key int64) (rspInfo *Entity) {
	r, err := Model.FindOne(g.Map{"aid": key})
	if err != nil {
		g.Log().Line().Errorf("获取数据失败 [fun:GetOne] [err:%v]", err.Error())
		return nil
	} else {
...
// 获取列表数据
func GetAllMany(condition g.Map) (rows []*Entity) {
	// 获取db
	rows, err := Model.FindAll(condition)
	if err != nil {
		g.Log().Errorf("获取数据失败 [err:%v]", err.Error())
		return
	}
...
func ModifyBase(updateInfo interface{}, info *Entity) error {
	r, err := Model.Data(updateInfo).Where(g.Map{"aid": info.Aid}).Update()
	if err != nil {
