...
// arModel is a active record design model for table t_admin operations.
type arModel struct {
	gmvc.M
}

var (
	// Table is the table name of t_admin.
	Table = "t_admin"
	// Model is the model object of t_admin.
	Model = &arModel{g.DB("default").Table(Table).Safe()}
	// Columns defines and stores column names for table t_admin.
	Columns = struct {
		Id          string //                                   
        Aid         string // 管理员唯一标识                    
        Name        string // admin昵称                         
        Phone       string // admin手机号码（账号）             
        Logo        string // admin头像                         
        Sign        string // 个性签名                          
        Pw          string // 密码                              
        Salt        string // 动态盐                            
        Status      string // admin状态（默认0，1在职，2离职）  
        RegisterIp  string //                                   
        CreateTime  string //                                   
        ModifyTime  string //
	}{
